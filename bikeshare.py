...
def get_filters():
    ...
    while True:
        ...
        except (TypeError, ValueError): # This should take care of any typos or unacceptable entries.
    ...
    while True:
        try:
            # Get user choice on how the dataframe is going to be filtered
            ...
            if filter_choice == 'month':
                ...
                try:
                    ...
                    if month not in ['january', 'february', 'march', 'april', 'may', 'june']:
                        ...
                        # All days are going to be included
                        day = 'all'
                        all_done = 'yes'
                        break
                ...
                # If no filter is applied, all months and days will be used.
                month = 'all'
                day = 'all'
                all_done = 'yes'
                print('\n')
...
def station_stats(df):
    """
    Displays statistics on the most popular stations and trip interval.
    """

    print('\nCalculating The Most Popular Stations and Trip...\n')
    start_time = time.time()

    # Display most commonly used start station and the number of time used.
    ...
    # Display most commonly used end station and the number of times used.
    ...
    # Display most frequent combination of start station and end station trip and the count of the trip.
...
def trip_duration_stats(df):
    ...
    tot_travel_time = datetime(1,1,1) + pd.to_timedelta(df['Trip Duration'].sum(), unit = 's')
    print('Total combined travel time is {} hours, {} minutes, and {} seconds.'.format(tot_travel_time.hour, tot_travel_time.minute, tot_travel_time.second))

    # Display mean travel time.
    avg_travel_time = datetime(1,1,1) + pd.to_timedelta(df['Trip Duration'].mean(), unit = 's')
    print('An average trip lasted {} minutes and {} seconds.'.format(avg_travel_time.minute, avg_travel_time.second))
...
def user_stats(df):
    ...
    except KeyError: # When no gender data are present as in Washington, display the appropriate message.
        print('This dataframe does not contain gender data, so no gender stats is given.')

    # Display earliest, most recent, and most common year of birth.
    # Also the count for the most common year is included
    try:
